...
class SceneC extends THREE.Scene{
    ...
    constructor(renderer, camera){
        ...
        if(active_index>=0){
            ...
            active_canvas.removeEventListener('touchmove', handleTouchMoveC, false); //タッチデバイスをなぞったときhandleTouchMoveを発火
            active_canvas.removeEventListener('touchend', handleTouchEndC, false);   //タッチデバイスから指を離したときhandleTouchEndを発火
        }
        ...
        active_canvas.addEventListener('touchmove', handleTouchMoveC, false); //タッチデバイスをなぞったときhandleTouchMoveを発火
        active_canvas.addEventListener('touchend', handleTouchEndC, false);   //タッチデバイスから指を離したときhandleTouchEndを発火
...
function createOrthographicCameraC(optiona){
    ...
    if(width > height){
        newcamera.left = - optiona.range * aspectratio;
        newcamera.right = optiona.range * aspectratio;
        newcamera.top = optiona.range;
        newcamera.bottom = -optiona.range;
    }else{
        newcamera.left = - optiona.range;
        newcamera.right = optiona.range;
        newcamera.top = optiona.range / aspectratio;
        newcamera.bottom = - optiona.range / aspectratio;
    }
    newcamera.position.set(optiona.pos[0], optiona.pos[1], optiona.pos[2]);
    newcamera.up.set(optiona.up[0], optiona.up[1], optiona.up[2]);
    newcamera.up.normalize();
